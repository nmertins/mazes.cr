...
describe "Mazes::Distances" do
  ...
  it "can give the shortest path between two cells, given that the distances have already been calculated" do
    ...
    # |   |       | 8 |   |
    # *---*---*---*   *   *
    # | c   b   a   9 |   |   (a = 10, b = 11, c = 12)
    # *---*---*---*---*---*

    knownShortestPath = [] of Mazes::Cell

    # row 1
    cell = grid[0, 0].as(Mazes::Cell)
    knownShortestPath << cell
    ...
    root = grid[0, 0].as(Mazes::Cell)
    goal = grid[4, 0].as(Mazes::Cell)
    distances = root.distances
    shortestPath = distances.path_to(goal)

    shortestPath.cells.size.should eq 13
    (knownShortestPath & shortestPath.cells).size.should eq 13
  end
end
